...
public class Bdd {
    ...
    public static void connect() {
        ...
        for (int i=0; i<t4.length; i++) {
            String[] champs = t4[i].split(";"); neo4j.run("CREATE (:Destination{code:'"+champs[0]+"',ville:'"+champs[1]+"'})");
            for (int j=2; j<champs.length; j+=2) { neo4j.run("MATCH (d:Depart),(a:Destination{code:'"+champs[0]+"'}) CREATE (d)-[:"+champs[2]+"{compagnie:'"+champs[3]+"'}]->(a)");
            }
        }
        for (int i=0; i<t5.length; i+=4) {
            neo4j.run("MATCH (d{code:'"+t5[i]+"'}),(a{code:'"+t5[i+1]+"'}) CREATE (d)-[:"+t5[i+2]+"{compagnie:'"+t5[i+3]+"'}]->(a)");
        }
    }
}